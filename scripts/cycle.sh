...
if ! npx tsx src/scripts/db-migrate.ts > /tmp/cycle-migrate.out 2>&1; then
...
if ! npx tsx src/position-watcher.ts > /tmp/cycle-watcher.out 2>&1; then
...
if [ "$TOP_OF_HOUR" = "1" ]; then
  ...
  if [ "$ENTER_COUNT" -gt 0 ] 2>/dev/null; then
    ...
    if ! npx tsx src/auto-execute.ts > /tmp/cycle-auto-exec.out 2>&1; then
