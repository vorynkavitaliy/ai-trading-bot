/**
 * LIVE-MIRROR BACKTEST — все параметры live зашиты внутри этого файла.
 *
 * Цель: один файл, все настройки видны на ладони, никаких импортов кроме движка
 * и фабрик стратегий. Если что-то не совпадает с live — это в этом файле, а не
 * где-то ещё.
 *
 * Источник чисел: snapshot vault/Snapshots/live-strategy-2026-05-29.json
 * Соответствует фактическому коду src/runtime/pair-strategies.ts +
 * src/runtime/risk-guard.ts на коммите 3232491.
 *
 * Usage: npx tsx src/backtest/cli/live-mirror-backtest.ts [days=365]
 */
...
// ═══════════════════════════════════════════════════════════════════════
//   1. LIVE PARAMETERS (зашитые)
// ═══════════════════════════════════════════════════════════════════════

const LIVE_RISK_PCT = 0.5;                    // pair-strategies.ts:31
const LIVE_LEVERAGE = 10;                     // config.ts:25 default
const LIVE_MAX_PARALLEL = 6;                  // risk-guard.ts:20

const LIVE_SCALED_IN_FIXED = {
  nEntries: 3,                                // pair-strategies.ts:42
  spacingAtr: 0.5,                            // pair-strategies.ts:43
  tpAtrMult: 2.0,                             // pair-strategies.ts:44
  sizingMode: 'dca_boost' as const,           // pair-strategies.ts:45
  dcaBoostDecay: 0.5,                         // pair-strategies.ts:46
  tpRecomputeOnFill: false,                   // pair-strategies.ts:47
};

const LIVE_BACKTEST_KNOBS = {
  startEquity: 668_000,                       // фактический total equity 4 prop subkeys
  slippagePct: parseFloat(process.env.SLIP ?? '0.25'),    // SLIP=0.05 для оптимистичного, дефолт 0.25 worst-case
  takerFeeRate: 0.00055,                      // Bybit perpetual taker
  makerFeeRate: 0.0002,                       // Bybit perpetual maker
  leverage: LIVE_LEVERAGE,
  decisionTf: '240m' as const,                // CG-fade strategies use 4H
  tp1SlMode: 'no_move' as const,              // tp1==tp2 single-target, SL не двигается
  bePlusBufferPct: 0.10,
  riskPctBase: LIVE_RISK_PCT,
};

// ═══════════════════════════════════════════════════════════════════════
//   2. LIVE UNIVERSE — 10 пар v5 (ребаланс 2026-05-27)
// ═══════════════════════════════════════════════════════════════════════
...
const LIVE_PAIRS: LivePair[] = [
  // S4 funding+TA confluence — pctHi 0.70, pctLo 0.30, оба тренда (default в фабрике)
  { symbol: 'SOLUSDT',  family: 'S4',
    strategy: fundingTaConfluence({ riskPct: LIVE_RISK_PCT, scaledIn: LIVE_SCALED_IN_FIXED }) },

  // S2 ls-top-pos fade + BTC trend (pctHi 0.85, pctLo 0.15)
  ...
  // S3 funding fade (default pctHi 0.75, pctLo 0.25, оба тренда)
...
// ═══════════════════════════════════════════════════════════════════════
//   3. POST-HOC PORTFOLIO KILL SWITCHES (mirror risk-guard.ts)
// ═══════════════════════════════════════════════════════════════════════
//   Каждая пара backtested ISOLATED. После этого применяем сквозные
//   ограничения портфеля: cap parallel positions, daily/total kill switches.
//   Сделки, что не прошли gate, удаляются из P&L.
...
function applyPortfolioKills(
  ...
  for (const ev of events) {
    ...
    if (ev.kind === 'entry') {
      ...
      // total kill -8, hard kill -4, soft kill -2.5 — все блокируют entry
...
async function main() {
  ...
  console.log('\n═══════════════════════════════════════════════════════════════════════');
  console.log('  LIVE-MIRROR BACKTEST');
  console.log(`  Source: всё зашито в этом файле (см. LIVE_PAIRS, LIVE_BACKTEST_KNOBS)`);
  ...
  console.log(`  Scaled-in: ${LIVE_SCALED_IN_FIXED.nEntries} слота × spacing ${LIVE_SCALED_IN_FIXED.spacingAtr}·ATR · dca_boost decay ${LIVE_SCALED_IN_FIXED.dcaBoostDecay} (eff риск/пара @ full DCA = ${(LIVE_RISK_PCT * 1.75).toFixed(3)}%)`);
  console.log(`  Fees: taker ${LIVE_BACKTEST_KNOBS.takerFeeRate*100}% / maker ${LIVE_BACKTEST_KNOBS.makerFeeRate*100}%  |  Slip: ${LIVE_BACKTEST_KNOBS.slippagePct}% (consv)`);
  console.log(`  Period: ${new Date(startTs).toISOString().slice(0,10)} → ${new Date(now).toISOString().slice(0,10)} (${days}d)`);
  console.log('═══════════════════════════════════════════════════════════════════════\n');
  ...
  console.log(`\n=== PORTFOLIO KILLS APPLIED (cap-${LIVE_MAX_PARALLEL} + daily DD switches) ===`);
  ...
  console.log('\n=== PER-PAIR (full window, ДО portfolio kills) ===');
  ...
  console.log('\n=== MONTHLY (FULL, fixed risk на $668k) ===');
  const months = Object.keys(fullAgg.monthly).sort();
  let runEq = LIVE_BACKTEST_KNOBS.startEquity;
  let posMonths = 0;
  for (const m of months) {
    ...
    const sign = pnl >= 0 ? '+' : '';
    console.log(`  ${m}: ${sign}${pnl.toFixed(0).padStart(8)} (${pct.toFixed(2).padStart(6)}%) → equity ${runEq.toFixed(0)}`);
  }
