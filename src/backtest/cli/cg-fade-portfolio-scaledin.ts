/**
 * CG-fade portfolio backtest WITH scaled-in entry (3 limits, spacing 0.6·ATR, TP 0.8·ATR).
 *
 * Mirror of cg-fade-portfolio.ts but every pair-strategy gets `scaledIn` enabled.
 * Compares end-to-end portfolio metrics against the production single-entry config.
 *
 * Used as the engine-level validation of the post-hoc simulation from
 * portfolio-scaled-in.ts. Expected: similar (or better) numbers when run
 * through the real engine code paths (not the simplified simulation).
 */
...
const SCALED_IN = { nEntries: 3, spacingAtr: 0.6, tpAtrMult: 0.8 };

interface PairCfg { pair: string; strategy: Strategy; }

const TIER1: PairCfg[] = [
  { pair: 'BTCUSDT',  strategy: lsTopPositionFade({ pctHi: 0.85, pctLo: 0.15, usePairTrend: true,  useBtcTrend: false, slAtrMult: 1.5, tpAtrMult: 2.0, maxHoldBars: 12, riskPct: 0.5, scaledIn: SCALED_IN }) },
  { pair: 'TAOUSDT',  strategy: fundingFade({ scaledIn: SCALED_IN }) },
  { pair: 'INJUSDT',  strategy: lsTopPositionFade({ pctHi: 0.85, pctLo: 0.15, usePairTrend: false, useBtcTrend: true,  slAtrMult: 1.5, tpAtrMult: 2.0, maxHoldBars: 12, riskPct: 0.5, scaledIn: SCALED_IN }) },
  { pair: 'ATOMUSDT', strategy: fundingFade({ scaledIn: SCALED_IN }) },
  { pair: 'ARBUSDT',  strategy: fundingFade({ scaledIn: SCALED_IN }) },
  { pair: 'XRPUSDT',  strategy: fundingTaConfluence({ scaledIn: SCALED_IN }) },
  { pair: 'LTCUSDT',  strategy: lsTopPositionFade({ pctHi: 0.85, pctLo: 0.15, usePairTrend: false, useBtcTrend: true, slAtrMult: 1.5, tpAtrMult: 2.0, maxHoldBars: 12, riskPct: 0.5, scaledIn: SCALED_IN }) },
];
...
async function main() {
  ...
  console.log(`=== Tier-1 portfolio WITH SCALED-IN — ${new Date().toISOString()} ===`);
  ...
  console.log(`ScaledIn: nEntries=${SCALED_IN.nEntries} spacingAtr=${SCALED_IN.spacingAtr} tpAtrMult=${SCALED_IN.tpAtrMult}\n`);
  ...
  // Portfolio simulation: shared compounding equity
