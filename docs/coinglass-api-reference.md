...
| CG signal / family | Archetype tested | Verdict | One-line why | Memory pointer |
|---|---|---|---|---|
| Aggregated **CVD** (cross-exch taker buy/sell, BTC) | confluence **filter** — drop a fade when 24h order-flow strongly opposes it | **REJECTED (cooldown artifact)** | best lead of the combo search; engine pass looked good (book +2.16pp OOS, BTC-standalone WR 50→53%) but the cap-4 cooldown-sensitivity sweep KILLED it: Δ flips to **−4.18pp at any-close 6h / −1.93pp at SL 6h** (±50% perturb) → benefit exists only at the exact live cooldown constants = path-dependent re-timing artifact, not robust loser-removal. Do NOT deploy. | `project_cvd_filter_marginal`, `project_combo_round1_2026_06_09` |
